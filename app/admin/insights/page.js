...
export default function AdminInsights() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        setDebugScan((p) => ({
          ...p,
          errors: [...(p.errors || []), `Count failed: ${String(e?.message || e)}`],
        }));
      }
      try {
        const lq = query(
      ...
      // Abandoned carts query kept for potential future use
      try {
        const cutoff = Timestamp.fromDate(
          new Date(Date.now() - 24 * 60 * 60 * 1000)
        );
        await getDocs(
          query(collection(db, "users"), where("cart.updatedAt", "<", cutoff))
        );
      } catch {
        // ignore for now
      }
  ...
  const topProducts = useMemo(() => {
    ...
    orders.forEach((o) =>
      (o.items || []).forEach((it) => {
        const key = String(
          it.productId || it.id || it.name || it.title || "unknown"
        );
        const prev =
          map.get(key) || {
            name: it.name || it.title || key,
            qty: 0,
            revenue: 0,
          };
  ...
  const darkBadge =
    "text-[12px] px-2 py-0.5 rounded bg-neutral-800 text-neutral-200 border border-neutral-700";
  ...
  return (
    ...
    >
      <div className="max-w-7xl mx-auto">
        ...
        <div
          className={`mb-6 rounded-xl px-3 py-3 flex flex-wrap items-center gap-3 ${panel}`}
        >
          ...
          {liveBlocked && (
            <span
              ...
              title="Falling back to one-time fetch (rules block live read)"
            >
        ...
        {debugOpen && (
          <div className={`mb-6 rounded-2xl overflow-hidden ${panel}`}>
            <div className={`px-4 py-3 text-base font-semibold ${headerBar}`}>
      ...
      <table className="w-full text-sm">
        ...
        <tbody className="tabular-nums">
          {rows.map((r, idx) => (
            <tr
              key={idx}
              className={`${zebra[idx % 2]} border-b border-orange-200/60 dark:border-neutral-800`}
            >
              {r.c.map((cell, i) => (
                <td
                  key={i}
...
function downloadOrdersCSV(orders) {
  ...
  a.download = "insights-orders.csv";
  a.click();
...
function downloadTopCSV(top, period) {
  const rows = [
    ...
    ...top.map((p) => [p.name, p.qty, money(p.revenue)]),
  ];
  const csv = rows
    .map((r) => r.map((x) => `"${String(x).replace(/"/g, '""')}"`).join(","))
    .join("\n");

...
export default function AdminInsights() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const sinceTs = Timestamp.fromDate(
      ...
      try {
        ...
        dbgErrors.push(
          `Top-level period query failed: ${String(e?.message || e)}`
        );
      }
      ...
      for (const name of names) {
        try {
          ...
          const snap = await getDocs(
            query(collectionGroup(db, name), limit(50))
          );
          ...
          dbgErrors.push(
            `CG scan '${name}' failed: ${String(e?.message || e)}`
          );
        }
      }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-10 text-neutral-900">
      <div className="mb-3 flex items-end gap-2">
        <div>
          ...
          <p className="text-sm text-neutral-600">
            Quick view of orders, trends and health.
          </p>
        </div>
        <button
      ...
      {/* toolbar */}
      <div
        className={`flex flex-wrap items-center gap-3 mb-6 rounded-xl border-2 border-black ${cardBg} px-3 py-3 shadow-[0_2px_0_#000]`}
      >
        ...
        <select
          id="period"
          className="rounded border-2 border-black bg-gray-50 px-3 py-2 text-sm focus:outline-none"
          value={period}
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            onClick={downloadCSV}
            className="px-3 py-2 bg-gray-50 border-2 border-black rounded text-sm font-semibold shadow-[0_2px_0_#000] active:translate-y-[1px]"
          >
            Export Orders CSV
          </button>
          <button
      ...
      {debugOpen && (
        <div
          className={`mb-6 rounded-2xl border-2 border-black ${cardBg} shadow-[0_3px_0_#000]`}
        >
          <div
            className={`text-base font-semibold px-4 py-3 border-b-2 border-black ${cardHdr}`}
          >
          ...
          <div className="p-4 text-sm">
            <div className="mb-2">
              <div className="font-semibold">
                Top-level <code>/orders</code> count:
              </div>
              <div>{debugScan.topLevelCount ?? "—"}</div>
            </div>
            <div className="mb-2">
              <div className="font-semibold">
                Subcollection scans (first 5 each):
              </div>
              {debugScan.cg.length === 0 ? (
                <div className="text-neutral-700">
                  No subcollection results yet.
                </div>
              ) : (
                <ul className="list-disc pl-5 space-y-2">
                  {debugScan.cg.map((g) => (
                    <li key={g.name}>
                      <span className="font-medium">{g.name}</span> — found{" "}
                      <b>{g.count}</b>
                      <div className="mt-1 grid sm:grid-cols-2 gap-2">
                        {g.samples.map((s, i) => (
                          <div
                            key={i}
                            className="rounded border border-black/20 bg-white p-2 overflow-auto"
                          >
                              ...
                              {JSON.stringify(
                                {
                                  id: s.id,
                                  createdAt:
                                    typeof s.createdAt === "object" &&
                                    s.createdAt?.seconds
                                      ? `Timestamp(${s.createdAt.seconds})`
                                      : s.createdAt,
                                  total: s.total,
                                  method: s.method,
                                  itemsLen: s.itemsLen,
...
function KPI({ label, value, cardBg }) {
  return (
    <div
      className={`rounded-2xl border-2 border-black ${cardBg} p-4 shadow-[0_3px_0_#000]`}
    >
...
function Card({ title, children, cardBg, cardHdr }) {
  return (
    <div
      className={`rounded-2xl border-2 border-black ${cardBg} shadow-[0_3px_0_#000]`}
    >
      <div
        className={`text-base font-semibold px-4 py-3 border-b-2 border-black ${cardHdr}`}
      >

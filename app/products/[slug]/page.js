...
export default async function ProductDetail({ params }) {
  ...
  if (!product) {
    return <main style={{ padding: 16 }}><h2>Product not found.</h2></main>;
  }

  return (
    <main style={{ maxWidth: 1000, margin: "0 auto", padding: 16, color: "#fff" }}>
      <p style={{ marginBottom: 12 }}>
        <Link href="/products" style={{ color: "#7aa2ff" }}>← Back to Products</Link>
      </p>
        ...
        <div>
          ...
          {product.category && (
            <ul style={{ marginTop: 12, lineHeight: 1.7, color: "#ddd" }}>
              ...
              {product.origin && <li><strong>Origin:</strong> {product.origin}</li>}
              {product.abv && <li><strong>ABV:</strong> {product.abv}</li>}
              {product.size && <li><strong>Size:</strong> {product.size}</li>}
            </ul>
          )}

          {product.description && (
            <p style={{ marginTop: 12, color: "#ddd" }}>{product.description}</p>
          )}
